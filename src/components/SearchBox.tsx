...
export default function SearchBox() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parsed.type === 'track' && parsed.id) {
      // 直接跳转到 canvas 页面
      router.push(
        `${withLocalePathname("/canvas", locale)}?link=${encodeURIComponent(
          query.trim(),
        )}`,
      );
    } else if (parsed.type === 'artist' && parsed.id) {
      // 艺术家页面
      router.push(withLocalePathname(`/artist/${parsed.id}`, locale));
    } else if (parsed.type === 'search') {
      // 搜索查询
      router.push(
        `${withLocalePathname("/search", locale)}?q=${encodeURIComponent(
          query.trim(),
        )}`,
      );
    } else {
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-2xl mx-auto">
      <div className="relative">
          ...
          <input
            ...
            placeholder={locale === "zh" ? "粘贴歌曲链接" : "Paste a track link"}
      ...
      <div className="mt-4 flex justify-center">
        ...
        >
          ...
          {locale === "zh" ? "打开 Spotify" : "Open Spotify"}
        </button>
      </div>
    </form>
  );
